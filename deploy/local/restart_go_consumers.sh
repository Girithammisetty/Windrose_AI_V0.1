#!/usr/bin/env bash
# Rebuild + restart the Go services that run go-common Kafka consumer groups, so
# they pick up the group-namespaced dedup fix (libs/go-common/kafka: the Redis
# idempotency claim is now scoped by consumer group, not global — without it,
# groups sharing one Redis silently dropped each other's events).
#
# case-service was already rebuilt separately; this covers the rest of the
# consumer fleet: realtime-hub, chart-service, usage-service, audit-service,
# notification-service. Each start_* rebuilds its binary from source (picking up
# the new go-common) and re-boots it.
#
# Usage: deploy/local/restart_go_consumers.sh
set -uo pipefail
cd "$(dirname "$0")"
E2E="$(cd ../e2e && pwd)"
...
say "stopping realtime-hub, chart, usage, audit, notification"
stop_port "$PORT_REALTIME" realtime
stop_port "$PORT_CHART" chart
stop_port "$PORT_USAGE" usage
stop_port "$PORT_AUDIT" audit
stop_port "$PORT_NOTIFICATION" notification

say "restarting realtime-hub"
start_realtime
say "restarting chart-service"
start_chart
say "restarting usage-service"
start_usage
say "restarting audit-service"
start_audit
say "restarting notification-service"
start_notification
ok "go-common consumer fleet restarted with the group-namespaced dedup fix"
