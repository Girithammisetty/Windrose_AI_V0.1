"""Auth middleware: JWT for /api/v1, SPIFFE allowlist for /internal/v1 + the CI plane."""
...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # The CI plane accepts either an allowed SPIFFE identity (mTLS) or a JWT.
        spiffe = request.headers.get(request.app.state.settings.spiffe_header, "")
        if spiffe and spiffe in request.app.state.settings.internal_allowed_spiffe:
