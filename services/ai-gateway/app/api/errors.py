...
from app.domain.errors import AppError, RateLimited
from app.utils import uuid7

logger = logging.getLogger(__name__)


def error_response(status: int, code: str, message: str, trace_id: str,
                   details=None, headers: dict | None = None) -> JSONResponse:
    body = {"error": {"code": code, "message": message, "trace_id": trace_id}}
    if details is not None:
        body["error"]["details"] = details
    all_headers = {"X-Trace-Id": trace_id, **(headers or {})}
    return JSONResponse(body, status_code=status, headers=all_headers)
