...
Pins dataset-service's app.events.envelope.make_envelope against the SAME
...
def test_envelope_matches_shared_conformance_validator():
    ...
    ds_id = "018f0000-0000-7000-8000-00000000000a"
    env = make_envelope(
        event_type="dataset.created",
        tenant_id=TENANT_A,
        actor={"type": "user", "id": "u-1"},
        resource_urn=f"wr:{TENANT_A}:dataset:dataset/{ds_id}",
        payload={"name": "orders", "workspace_id": "ws-1"},
        via_agent=None,
        trace_id="tr-1",
    )
