...
semantic-service pushes its action manifest to rbac's idempotent registration
...
# Semantics: model.create = POST /models; model.update = model/version edits +
# submit + bootstrap (and gates draft compiles, BR-2); model.approve =
# approve/reject; compile.execute = metric/chart compilation; verified_query
# create/update/approve mirror the VQ lifecycle.
MANIFEST: list[str] = [
    "semantic.model.read",
...
def _mint_service_token(settings) -> str:
    now = int(time.time())
    claims = {
        "sub": "svc:semantic-service", "typ": "service",
...
async def register_actions(settings) -> bool:
    if not settings.rbac_url or not settings.register_signing_key_pem:
        logger.info("semantic action registration skipped (rbac_url/signing key unset)")
        return False
    try:
        token = _mint_service_token(settings)
    except Exception as exc:  # noqa: BLE001
        logger.warning("semantic action registration: token mint failed: %s", exc)
        return False
        ...
        for attempt in range(10):
            try:
                ...
                if resp.status_code == 200:
                    logger.info("semantic action catalog registered (%d actions)",
                                len(MANIFEST))
                    return True
    ...
    logger.warning("semantic action registration failed: %s", last)
    return False
