...
// (MASTER-FR-032). The real adapter is KafkaPublisher (gocommon.go); InMemory
// below is the unit-test double only.
type Publisher interface {
	Publish(ctx context.Context, envs []Envelope) error
}

// InMemory collects published envelopes (unit tests / broker-less dev).
