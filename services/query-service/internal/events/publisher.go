...
// Publisher is the outbound event port. Producers are idempotent by
// event_id (MASTER-FR-032).
type Publisher interface {
	Publish(ctx context.Context, envs []Envelope) error
}

// InMemory collects published envelopes (tests, local dev).
...
// The real query.events.v1 publisher lives in gocommon.go (KafkaPublisher,
// backed by the shared libs/go-common Kafka producer over Redpanda). InMemory
// above is the unit-test double only.

// OutboxSource is what the relay drains (implemented by both stores).
...
// Relay drains the transactional outbox to the publisher (MASTER-FR-034:
// never emit before commit — only committed rows are visible here).
type Relay struct {
	Source    OutboxSource
	Publisher Publisher
